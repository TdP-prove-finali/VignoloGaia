...
class Model:
    ...
    def load_csv_file(self, file_path: str) -> int:
        ...
        self._csv_rows = []
        field_names = [
            "Sede", "Ufficio", "Attivita", "Nome Operatore", "ID_Operatore",
    ...
    def format_date(self, raw_date: str) -> str:
        """Converte la data nel formato corretto per MySQL."""
        if not raw_date:
            return None
        for f_str in ("%d/%m/%Y", "%Y-%m-%d"):
            try:
                return datetime.strptime(raw_date, f_str).strftime("%Y-%m-%d")
            except ValueError:
                continue
        return None
    ...
    def get_csv_total(self) -> int:
        """Ritorna il numero totale di righe CSV."""
        return self._csv_total
    ...
    def import_csv_row(self, index: int) -> bool:
        ...
        return DAO.insert_csv_row(row, fmt_date)
